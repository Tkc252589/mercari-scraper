...
function findChromiumExecutable() {
      // Try playwright's own path first
  try {
          var p = chromium.executablePath();
          if (fs.existsSync(p)) {
                    console.log('Using playwright executablePath:', p);
                    return p;
          }
          console.log('Playwright path not found:', p);
  } catch(e) {
          console.log('executablePath() error:', e.message);
  }
      // Search in common Render/playwright cache directories
  var searchDirs = [
      ...
      for (var i = 0; i < searchDirs.length; i++) {
              var baseDir = searchDirs[i];
              if (!fs.existsSync(baseDir)) continue;
              var entries = fs.readdirSync(baseDir);
              console.log('Playwright cache dirs found:', entries);
              for (var j = 0; j < entries.length; j++) {
                        var entry = entries[j];
                        var subDirs = ['chrome-linux64', 'chrome-linux', 'chrome-mac', 'chrome-win'];
                        for (var k = 0; k < subDirs.length; k++) {
                                    for (var m = 0; m < candidates.length; m++) {
                                                  var fullPath = path.join(baseDir, entry, subDirs[k], candidates[m]);
      ...
      throw new Error('Chromium executable not found in any known location');
}
...
async function getBrowser() {
      ...
      if (!browserInstance) {
              var execPath = findChromiumExecutable();
...
app.get('/debug', function(req, res) {
      var info = {};
      try { info.playwrightPath = chromium.executablePath(); } catch(e) { info.playwrightPathError = e.message; }
      var dirs = ['/opt/render/.cache/ms-playwright', '/root/.cache/ms-playwright'];
      info.dirs = {};
      dirs.forEach(function(d) {
              try { info.dirs[d] = fs.readdirSync(d); } catch(e) { info.dirs[d] = 'not found'; }
      });
      res.json(info);
});
